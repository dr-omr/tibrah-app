...
// TIBRAH — Premium Native Booking v5 "Apple-tier"
// Micro-animations · Haptics · Glass cards · Confetti success
...
import {
    ...
    Instagram, Music2, Lock, Sparkles,
...
// ─── CONFIG ───────────────────────────────────────────────────────
const DR_PHONE = '967771447111';
const DR_PHOTO = 'https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/69287e726ff0e068617e81b7/9185440e5_omar.jpg';
const DR_IG    = 'dr.omr369';
const DR_TT    = 'dr.omr369';

const SESSIONS = [
    {
        id: 'consultation', emoji: '🩺',
        label: 'جلسة تشخيصية شاملة',
        tagline: 'نكتشف السبب الجذري لمشكلتك',
        price: '٣٥ ر.س', original: '٣٥٠ ر.س', duration: '٤٥ د',
        color: '#0d9488', glow: 'rgba(13,148,136,0.22)',
        badge: 'الأكثر طلباً',
    },
    {
        id: 'emotional', emoji: '💛',
        label: 'الطب الشعوري',
        tagline: 'الصلة بين مشاعرك وجسمك',
        price: '٢٥٠ ر.س', original: '', duration: '٤٠ د',
        color: '#d97706', glow: 'rgba(217,119,6,0.18)',
        badge: 'جديد',
    },
    {
        id: 'frequencies', emoji: '🎵',
        label: 'جلسة الترددات العلاجية',
        tagline: 'علاج تكميلي بالموجات الصوتية',
        price: '٢٠٠ ر.س', original: '', duration: '٣٠ د',
    ...
    },
    {
        id: 'followup', emoji: '💚',
        label: 'متابعة دورية',
        tagline: 'تابع تطورك مع الدكتور',
        price: '١٥٠ ر.س', original: '', duration: '٢٠ د',
...
const SPRING_SMOOTH = { type: 'spring' as const, stiffness: 280, damping: 26 };
...
function ShimmerCard({ active, color, children, onClick }: {
    ...
    return (
        <motion.button
            ...
            style={{
                ...
                boxShadow: active
                    ? `0 8px 28px ${color}25, 0 1px 0 rgba(255,255,255,0.8) inset`
                    : '0 2px 10px rgba(0,0,0,0.055), 0 1px 0 rgba(255,255,255,0.9) inset',
            }}
        >
...
function FloatInput({ label, value, onChange, type = 'text', placeholder = '' }: {
        ...
        <div className="relative">
            <div className="relative rounded-[16px] transition-all overflow-hidden"
                style={{
                    ...
                    border: `1.5px solid ${focused ? '#0d9488' : 'rgba(0,0,0,0.08)'}`,
                    boxShadow: focused ? '0 0 0 3px rgba(13,148,136,0.1)' : '0 2px 6px rgba(0,0,0,0.04)',
                }}>
                <label
                    className="absolute right-4 pointer-events-none font-bold transition-all duration-200"
                    style={{
                        top: lifted ? '8px' : '50%',
                        transform: lifted ? 'none' : 'translateY(-50%)',
                        fontSize: lifted ? '10px' : '14px',
                        color: lifted ? (focused ? '#0d9488' : '#94a3b8') : '#94a3b8',
                    }}
                >
                    {label}
                </label>
                <input
                    ...
                    className="w-full px-4 pb-3 pt-6 text-[14px] font-medium outline-none bg-transparent"
                    style={{ color: '#0f172a' }}
                />
            </div>
        </div>
...
// ─── FIXED BOTTOM CTA ─────────────────────────────────────────────
function BottomCTA({ label, sublabel, disabled, loading, onClick }: {
    ...
    return (
        <div className="fixed bottom-0 inset-x-0 z-50 px-5 pb-8 pt-4"
            style={{ background: 'linear-gradient(to top, #F0FAF8 65%, rgba(240,250,248,0))' }}>
            <motion.button
                ...
                style={{
                    ...
                    background: disabled
                        ? 'rgba(0,0,0,0.08)'
                        : 'linear-gradient(135deg, #0d9488 0%, #0f766e 50%, #0d9488 100%)',
                    backgroundSize: '200% 100%',
                    boxShadow: disabled ? 'none' : '0 10px 32px rgba(13,148,136,0.4)',
                    color: disabled ? '#94a3b8' : '#fff',
                }}
                animate={!disabled ? { backgroundPosition: ['0% 0%', '100% 0%', '0% 0%'] } : {}}
            ...
            >
                {loading ? (
                    ...
                    <>
                        ...
                        {sublabel && <span className="text-[10px] font-medium opacity-70 mt-0.5">{sublabel}</span>}
                    </>
                )}
            </motion.button>
        </div>
...
function StepSession({ selected, onSelect, onNext }: {
    ...
    return (
        ...
        >
            <motion.p initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                className="text-[13px] font-bold text-slate-400 mb-5">
                ما الجلسة التي تحتاجها؟
            </motion.p>
                ...
                {SESSIONS.map((s, i) => {
                    ...
                    return (
                        <motion.div key={s.id}
                            ...
                            <ShimmerCard active={active} color={s.color}
                                onClick={() => { onSelect(s.id); haptic.selection(); }}>

                                <div className="flex items-center gap-4 px-4 py-4">
                                    {/* Emoji icon */}
                                    <motion.div
                                        className="w-[52px] h-[52px] rounded-[16px] flex items-center justify-center flex-shrink-0 text-[26px]"
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-0.5">
                                            ...
                                            {s.badge && (
                                                <motion.span
                                                    ...
                                                    className="flex-shrink-0 text-[9px] font-black text-white px-2 py-0.5 rounded-full"
                                        ...
                                        <div className="flex items-center gap-3 mt-2">
                                            ...
                                            {s.original && (
                                                <span className="text-[11px] text-slate-300 line-through">{s.original}</span>
                                            )}
                                    ...
                                    <motion.div
                                        ...
                                        style={{ background: active ? s.color : 'rgba(0,0,0,0.06)' }}
            ...
            <BottomCTA
                label={selected ? `التالي — اختر الموعد` : 'اختر جلسة للمتابعة'}
                disabled={!selected}
...
// ─── STEP 1: DATE & TIME ──────────────────────────────────────────
function StepDateTime({ form, setForm, onNext }: { form: any; setForm: any; onNext: () => void }) {
    ...
    return (
        <motion.div
            ...
            transition={SPRING_SMOOTH} className="pb-28">

            {/* Date row */}
            <p className="text-[13px] font-bold text-slate-400 mb-3">اختر اليوم</p>
            <div ref={scrollRef} className="flex gap-2.5 overflow-x-auto pb-3 mb-6 snap-x snap-mandatory"
                ...
                {days.map((d, i) => {
                    ...
                    return (
                        <motion.button key={i}
                            ...
                            whileTap={{ scale: 0.92 }}
                            onClick={() => { setForm((p: any) => ({ ...p, date: d })); haptic.selection(); }}
                            className="flex flex-col items-center py-3 px-3.5 rounded-[18px] flex-shrink-0 min-w-[64px] snap-start transition-all"
                            style={{
                                background: active ? '#0d9488' : 'rgba(255,255,255,0.94)',
                                boxShadow: active
                                    ? '0 8px 20px rgba(13,148,136,0.35)'
                                    : '0 2px 8px rgba(0,0,0,0.05)',
                            }}>
                            <span className={`text-[10px] font-bold ${active ? 'text-white/70' : 'text-slate-400'}`}>
                                {AR_DOW[d.getDay()]}
                            </span>
                            <span className={`text-[24px] font-black leading-tight ${active ? 'text-white' : 'text-slate-800'}`}>
                                {d.getDate()}
                            </span>
                            <span className={`text-[9px] mt-0.5 ${active ? 'text-white/60' : 'text-slate-400'}`}>
                                {format(d, 'MMM', { locale: ar })}
                            </span>
                            {active && (
                                <motion.div className="w-1.5 h-1.5 rounded-full bg-white mt-1.5"
            ...
            <div className="grid grid-cols-3 gap-2.5">
                {TIMES.map((slot, i) => {
                    if (!slot) return (
                        <div key={i} className="col-span-3 h-px my-1" style={{ background: 'rgba(0,0,0,0.05)' }} />
                    );
                    ...
                    return (
                        <motion.button key={slot}
                            ...
                            whileTap={{ scale: 0.91 }}
                            onClick={() => { setForm((p: any) => ({ ...p, time_slot: slot })); haptic.selection(); }}
                            className="py-3.5 rounded-[14px] text-[13px] font-bold transition-all relative overflow-hidden"
                            style={{
                                background: active ? '#0d9488' : 'rgba(255,255,255,0.94)',
                                color: active ? '#fff' : '#334155',
                                boxShadow: active ? '0 6px 16px rgba(13,148,136,0.3)' : '0 1px 4px rgba(0,0,0,0.05)',
                            }}>
                            {active && (
            ...
            <AnimatePresence>
                {canNext && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                        className="mt-5 flex items-center gap-2.5 px-4 py-3 rounded-[14px]"
                        ...
                        <p className="text-[12.5px] font-bold text-teal-700">
                            {format(form.date!, 'EEEE d MMMM', { locale: ar })} · {form.time_slot}
                        </p>
            ...
            <BottomCTA label="التالي — بياناتك" disabled={!canNext}
                onClick={() => { haptic.impact(); onNext(); }} />
        </motion.div>
    );
}
...
// ─── STEP 2: PATIENT INFO ─────────────────────────────────────────
function StepInfo({ form, setForm, onSubmit, isLoading }: {
    form: any; setForm: any; onSubmit: () => void; isLoading: boolean;
}) {
    const session = SESSIONS.find(s => s.id === form.session_type);
    const canBook = !!form.patient_name?.trim() && !!form.patient_phone?.trim();

    return (
        <motion.div key="step-info"
            initial={{ opacity: 0, x: 32 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -32 }}
            transition={SPRING_SMOOTH} className="pb-28">
            ...
            {session && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    style={{ background: `${session.color}0D`, border: `1.5px solid ${session.color}22` }}>
                    <span className="text-[22px]">{session.emoji}</span>
                    <div className="flex-1">
                        <p className="text-[13px] font-black text-slate-700">{session.label}</p>
                        <p className="text-[11px] text-slate-400">
                            {form.date ? format(form.date, 'd MMM', { locale: ar }) : ''} · {form.time_slot}
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-[16px] font-black" style={{ color: session.color }}>{session.price}</p>
            ...
            <div className="space-y-3.5">
                <FloatInput label="الاسم الكامل *" value={form.patient_name || ''}
                    onChange={v => setForm((p: any) => ({ ...p, patient_name: v }))}
                    placeholder="أحمد محمد العمري" />

                <FloatInput label="رقم الجوال (واتساب) *" value={form.patient_phone || ''}
                    onChange={v => setForm((p: any) => ({ ...p, patient_phone: v }))}
                    type="tel" placeholder="+967 7XX XXX XXX" />

                <FloatInput label="البريد الإلكتروني" value={form.patient_email || ''}
                    onChange={v => setForm((p: any) => ({ ...p, patient_email: v }))}
                    type="email" placeholder="اختياري" />

                {/* Concern textarea */}
                <div className="relative">
                    <div className="rounded-[16px] transition-all overflow-hidden"
                        style={{ background: 'rgba(255,255,255,0.95)', border: '1.5px solid rgba(0,0,0,0.08)' }}>
                        <p className="absolute top-3 right-4 text-[10px] font-bold text-slate-400 pointer-events-none">شكواك الصحية</p>
                        <textarea rows={3} value={form.health_concern || ''}
                            onChange={e => setForm((p: any) => ({ ...p, health_concern: e.target.value }))}
                            placeholder="اكتب باختصار ما تعاني منه..."
                            className="w-full px-4 pb-3 pt-7 text-[14px] font-medium outline-none resize-none bg-transparent"
                            style={{ color: '#0f172a' }}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                className="flex items-center gap-2 mt-4 px-4 py-2.5 rounded-[12px]"
                style={{ background: 'rgba(0,0,0,0.03)' }}>
                <Lock className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                <p className="text-[11px] text-slate-400 font-medium">بياناتك محمية ومشفرة ١٠٠٪ · لن نشاركها مع أحد</p>
            </motion.div>

            <BottomCTA
                label="أكد الحجز"
                sublabel={session ? `${session.label} · ${session.price}` : undefined}
                disabled={!canBook}
                loading={isLoading}
                onClick={onSubmit}
            />
...
// ─── STEP 3: SUCCESS ──────────────────────────────────────────────
function StepSuccess({ form }: { form: any }) {
    const session = SESSIONS.find(s => s.id === form.session_type);
    ...
    return (
        <>
            ...
            <motion.div
                ...
                transition={{ ...SPRING_SMOOTH, delay: 0.05 }}
                className="flex flex-col items-center pb-10 pt-4"
            >
                {/* Pulse success circle */}
                <div className="relative mb-6">
                    {[1,2,3].map(ring => (
                        <motion.div key={ring}
                            className="absolute inset-0 rounded-full"
                            style={{ border: `2px solid rgba(13,148,136,${0.15 / ring})` }}
                            animate={{ scale: [1, 1.5 + ring * 0.3], opacity: [0.6, 0] }}
                            transition={{ duration: 2, delay: ring * 0.25, repeat: Infinity, ease: 'easeOut' }}
                        />
                    ))}
                    <motion.div
                ...
                <motion.h2 initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                    className="text-[24px] font-black text-slate-800 mb-2">
                    تم استلام حجزك! 🎉
                </motion.h2>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}
                    className="text-[13px] text-slate-400 text-center mb-7 px-6 leading-relaxed">
                    سيتواصل معك د. عمر لتأكيد الموعد قريباً إن شاء الله
                </motion.p>

                {/* Booking card */}
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                    className="w-full p-5 rounded-[22px] mb-5"
                    style={{ background: '#fff', boxShadow: '0 6px 28px rgba(0,0,0,0.07)' }}>
                    {/* Doctor */}
                    <div className="flex items-center gap-3 pb-4 mb-4 border-b border-slate-100">
                        <img src={DR_PHOTO} alt="" className="w-11 h-11 rounded-[13px] object-cover object-top" />
                        <div>
                            <p className="text-[13px] font-black text-slate-800">د. عمر العماد</p>
                            <p className="text-[11px] text-teal-600 font-bold">الطب الوظيفي والتكاملي</p>
                        </div>
                        <div className="mr-auto flex items-center gap-1 px-2 py-1 rounded-full" style={{ background: 'rgba(16,185,129,0.1)' }}>
                            <motion.div className="w-1.5 h-1.5 bg-emerald-500 rounded-full"
                                animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} />
                    ...
                    {/* Details */}
                    {[
                        { l: 'الجلسة', v: `${session?.emoji} ${session?.label}` },
                        { l: 'التاريخ', v: dateStr },
                        { l: 'الوقت', v: form.time_slot },
                        { l: 'السعر', v: session?.price },
                    ].map((row, i) => (
                        <motion.div key={row.l} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                ...
                <motion.a initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
                    href={`https://wa.me/${DR_PHONE}?text=${encodeURIComponent(waMsg)}`}
                    ...
                    تحدث مع الدكتور الآن
                </motion.a>

                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
                    className="flex gap-3 w-full mb-4">
                    <a href={`https://instagram.com/${DR_IG}`} target="_blank" rel="noopener noreferrer"
                        ...
                        style={{ background: 'linear-gradient(135deg,#e1306c,#833ab4,#405de6)' }}>
                        <Instagram className="w-4 h-4" /> إنستغرام
                    </a>
                    <a href={`https://tiktok.com/@${DR_TT}`} target="_blank" rel="noopener noreferrer"
                        ...
                        style={{ background: 'linear-gradient(135deg,#111,#333)', color: '#fff' }}>
                        <Music2 className="w-4 h-4" /> تيكتوك
                    </a>
                </motion.div>

                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="w-full">
                    <Link href={createPageUrl('Home')}>
                        <button className="w-full h-[46px] rounded-[16px] text-[14px] font-bold text-slate-400"
...
export default function BookAppointment() {
    ...
    const [form, setForm] = useState({
        session_type: '', date: null as Date | null, time_slot: '',
    ...
    const mutation = useMutation({
        mutationFn: async (data: typeof form) => {
            ...
            const payload = {
                ...
                session_type_label: session?.label || '',
                status: 'pending',
                user_id: user?.id || 'guest',
            };
            if (user?.id) {
                try { await db.entities.Appointment.createForUser(user.id, payload); } catch {}
            }
    ...
    const handleSubmit = () => {
        if (!form.patient_name.trim() || !form.patient_phone.trim()) {
            toast.error('يرجى إدخال اسمك ورقم جوالك'); return;
        }
        mutation.mutate(form);
    };

    const STEP_LABELS = ['الجلسة', 'الموعد', 'بياناتك'];
    ...
    return (
        <div className="min-h-screen bg-[#F0FAF8]" dir="rtl">
            ...
            <motion.div
                ...
                style={{ background: 'rgba(240,250,248,0.85)', backdropFilter: 'blur(24px)' }}
            >
                <div className="flex items-center gap-3 py-3">
                    ...
                    <AnimatePresence mode="wait">
                        {step === 0 ? (
                            ...
                            <motion.button key="back-btn" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                                whileTap={{ scale: 0.88 }} onClick={goBack}
                                className="w-9 h-9 rounded-full flex items-center justify-center"
                                style={{ background: 'rgba(0,0,0,0.06)' }}>
                                <ArrowRight className="w-4 h-4 text-slate-600" />
                            </motion.button>
                    ...
                    <div className="flex-1">
                        <AnimatePresence mode="wait">
                            <motion.p key={`title-${step}`}
                                initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
                                transition={{ duration: 0.18 }}
                                className="text-[17px] font-black text-slate-800">
            ...
            <div className="px-4 pt-4 max-w-md mx-auto">
                <AnimatePresence mode="wait">
                    {step === 0 && (
                        <StepSession key="s0"
                            selected={form.session_type}
                            onSelect={id => setForm(p => ({ ...p, session_type: id }))}
                            onNext={() => setStep(1)} />
                    )}
                    {step === 1 && (
                        <StepDateTime key="s1" form={form} setForm={setForm} onNext={() => setStep(2)} />
                    )}
                    {step === 2 && (
                        <StepInfo key="s2" form={form} setForm={setForm}
